...
import {getCashOrderObject} from '../../common/util';
...
@Component({
  selector: 'app-create-cash-order',
  templateUrl: './create-cash-order.component.html',
  styleUrls: ['./create-cash-order.component.css'],
  providers: [SnackBarService]
})
export class CreateCashOrderComponent implements OnInit {
  cashOrderNo: Number;
  ...
  ngOnInit() {
    ...
    this.orderService.getCashOrderNo().subscribe(res => {
      this.dataLoading = false;
      this.cashOrderNo = res;  
  ...
  saveAndPrintOrder() {
    ...
    let cashOrderDate  = new Date(year + '-' + month + '-' + date);
    ...
    this.orderService.billingOrder = cashOrderObject;
      this.orderService.saveCashOrder(cashOrderObject).subscribe(res => {
        this.snackBarService.snackMessage('S', 'Cash Order Saved successfully! Generating cash order now...');
        printBill(printObject);
        // this.router.navigate(['/bill_print']);
      }, err => {
        this.snackBarService.snackMessage('E', 'There was some problem saving the cash order.');
      });
